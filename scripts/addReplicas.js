...
for (var cluster = 2, n = regions.length + 1; cluster < n; cluster++) {
    ...
    for (var i = 0, k = resp.nodes; i < k.length; i++) {
        if (k[i].nodeGroup == 'nosqldb') {
            ...
            resp = jelastic.env.control.ExecCmdById('${settings.mainEnvName}-1', session, ${@i.id}, toJSON([{"command": meetCommand, "params": ""}]), false, "root");
            if (resp.result != 0) { return resp; }
            resp = jelastic.env.control.ExecCmdById('${settings.mainEnvName}-1', session, ${@i.id}, toJSON([{"command": targetMasterIdCommand, "params": ""}]), false, "root");
            if (resp.result != 0) { return resp; }
