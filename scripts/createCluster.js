var regions = '${settings.regions}'.split(','), masterNodesString = "", createClusterCommand = "", resp,
...
for (var cluster = 1, n = regions.length + 1; cluster < n; cluster++) {
    var envInfo = jelastic.env.control.GetEnvInfo('${settings.mainEnvName}-' + cluster, session);
    if (envInfo.result != 0) {
        return envInfo;
    }
...
resp = jelastic.env.control.ExecCmdById('${settings.mainEnvName}-' + cluster, session, envInfo.nodes[0].id, toJSON([{"command": createClusterCommand, "params": ""}]), false, "root");
if (resp.result != 0) { return resp; }    
